	...
	def moveLocal(self):
		...
		source = '/tmp/Practice_Ayush/majordir/GIT/Helping-Hand'
                ...
                if not os.path.isdir(dest1):
                        ...
                        #est1='/tmp/Practice_Ayush/production/scripts'
                        os.mkdir(dest1)
			os.remove("/tmp/Practice_Ayush/majordir/GIT/Helping-Hand/DEV.tar")
                        print("Created new directory DEV,Also removed the existing tar")
			...
			if f == "DEV":
                ...
                tar = tarfile.open("/tmp/Practice_Ayush/majordir/GIT/Helping-Hand/DEV.tar", "w:tar")
                #print(os.listdir(directory))
                for filename in os.listdir(directory):
			if os.path.isdir(source+'/'+f):
                                #shutil.rmtree(dest1, True)
